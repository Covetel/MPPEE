...
my $target = shift
  || die
"Debe Suministrar el tipo de zona a probar (public/private) como primer argumento.\n";
...
my @exclude_attrs =
  qw(top dnsttl dnsclass objectclass zonename relativedomainname);
        ...
        if ( $entry->get_value("soarecord") ) {
            unless ( $attr ~~ @exclude_attrs ) {
                given ( $map_attr{$attr} ) {
                    when ("TXT") {
                        my @var =
                          map { s/["']//g; $_; } $entry->get_value($attr);
                        $pointers->{ $entry->get_value("zonename") }
                          ->{ $map_attr{$attr} } = [@var];
                    }
                    when ("MX") {
                    ...
                    default {
                        $pointers->{ $entry->get_value("zonename") }
                          ->{ $map_attr{$attr} } = [ $entry->get_value($attr) ];
                    }
                }
                push( @at, $map_attr{$attr} )
                  unless ( $map_attr{$attr} ~~ @at );
                $pointers->{ $entry->get_value("zonename") }->{"type_pointer"} =
                  [@at];
            }
        }
        else {
            unless ( $attr ~~ @exclude_attrs ) {
                given ( $map_attr{$attr} ) {
                    when ("CNAME") {
                        unless ( $entry->get_value($attr) =~ /\.$/ ) {
                            ...
                            my @cnames =
                              map { s/(.*$)/$1.$zone/; $_; }
                              $entry->get_value($attr);
                            $pointers->{ $entry->get_value("relativedomainname")
                                  . "."
                                  . $entry->get_value("zonename") }
                              ->{ $map_attr{$attr} } = [@cnames];
                        }
                        else {
                            $pointers->{ $entry->get_value("relativedomainname")
                                  . "."
                    ...
                    default {
                        my @var =
                          map { s/\s*$//; $_; } $entry->get_value($attr);
                        $pointers->{ $entry->get_value("relativedomainname")
                              . "."
                              . $entry->get_value("zonename") }
                          ->{ $map_attr{$attr} } = [@var];
                ...
                push( @at, $map_attr{$attr} )
                  unless ( $map_attr{$attr} ~~ @at );
                $pointers->{ $entry->get_value("relativedomainname") . "."
                      . $entry->get_value("zonename") }->{"type_pointer"} =
                  [@at];
            }
        }
    }
